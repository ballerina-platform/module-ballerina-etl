...
function standardizeDataFunc(record {}[] dataset, string fieldName, string[] standardValues, string modelName) returns json|Error {
    ...
    return getResponseFromModel(prompt, modelName);
...
function extractFromUnstructuredDataFunc(string dataset, string[] fieldNames, string modelName) returns json|Error {
    ...
    string prompt = string ` Extract relevant details from the given string array and map them to the specified fields. 
...
function getResponseFromModel(string prompt, string modelName) returns json|Error {
